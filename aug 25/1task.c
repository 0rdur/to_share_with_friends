...
int main(){ 
    ...
    // Calculate padding per row (must align to 4-byte boundaries)
    ...
    for (y = image->height - 1; y >= 0; y--){
        ...
        // Skip padding bytes at the end of each row in input file
    ...
    for (y = image->height - 1; y >= 0; y--){
        for (x = 0; x < image->width; x++){ 
            ...
            unsigned char gray = (unsigned char)(
                0.299 * image->data[y * image->width + x].r +
                0.587 * image->data[y * image->width + x].g +
                0.114 * image->data[y * image->width + x].b
            );

            // 2. Assign the SAME gray value to R, G, and B
        ...
        // 3. Write row padding bytes out to the file
